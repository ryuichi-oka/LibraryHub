...
## 4. 画面設計（MVP）
- `共通レイアウト`
  - ログイン後画面はヘッダーとサイドメニューを共通表示する
  - サイドメニューの表示項目はロール（管理者/一般利用者）に応じて切り替える
- `ログイン画面`
...
### 4.1 画面遷移制御
- 管理者専用画面（`/admin/**`）は管理者ロールのみ遷移可能とする
- 一般利用者が管理者専用画面へ遷移した場合はトップ画面（新着一覧画面）へリダイレクトする
- 未ログイン状態で管理者専用画面へ遷移した場合はログイン画面へリダイレクトする
...
- メール通知チャネル追加（FR-044）
- 社内SSO連携（FR-005）
  - 連携ポイント整理メモ: [sso-integration-memo.md](./development/sso-integration-memo.md)
